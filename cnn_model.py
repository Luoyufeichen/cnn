...
class CNN_Text(nn.Module):
    def __init__(self, args):
        ...
        if args.max_norm is not None:
            ...
            self.embed = nn.Embedding(V, D, max_norm=5, scale_grad_by_freq=True, padding_idx=1)
        else:
            print("max_norm = {} ".format(args.max_norm))
        ...
        if args.out_word_v is True:
            pretrain_embed = np.array(args.word_embed)
            xx = torch.from_numpy(pretrain_embed)
            print(xx)
            self.embed.weight.data.copy_(xx)
